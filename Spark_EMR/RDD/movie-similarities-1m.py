...
# 从hdfs 上创建RDD's Spark 会默认给hdfs上每一个block创建一个partition
# Create RDD's
# Read a text file from HDFS, a local file system s3n:// or hdfs://
# return it as an RDD of Strings
# 但是也可以用过textFile（）的minPartitions自定义 partition 数量，但必须比block大
data = sc.textFile("s3n://sundog-spark/ml-1m/ratings.dat")  # open data from S3
...
ratingsPartitioned = ratings.partitionBy(100)  ## partition 100
